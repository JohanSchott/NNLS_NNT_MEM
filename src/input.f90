...
subroutine getFirstIndices(inputfile,startindex,N,wn,Gn)
     implicit none
     character(len=800),intent(in) :: inputfile
     integer,intent(in) :: startindex
     integer,intent(in) :: N
     real(kind=16),intent(out),allocatable :: wn(:) !matsubara points
     complex(kind=16),intent(out),allocatable :: Gn(:) !Matsubara Green's function
